...
contract AssistDividendTracker is Ownable, DividendPayingToken, ERC20TokenRecover, IAssistDividendTracker {
    ...
    constructor(address dividendToken, address _parentToken)
        DividendPayingToken("Assist Dividend Tracker", "ASSTDT", dividendToken)
    {
        claimWait = 3600;
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount)
        ...
        onlyOwner
    {
        require(tokenAddress != dividendToken, "AssistDividendTracker: Cannot retrieve USDT");
    ...
    function _transfer(
        ...
        require(false, "AssistDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override(DividendPayingToken, IDividendPayingTokenInterface) {
        require(
            false,
            "AssistDividendTracker: Disabled. Use the 'claim' function on the main TetherMoon contract."
        );
    }

    function excludeFromDividends(address account) external override onlyOwnerOrDeployer {
        require(!excludedFromDividends[account], "AssistDividendTracker: Account already excluded");
    ...
    function includeInDividends(address account) external override onlyOwnerOrDeployer {
        require(excludedFromDividends[account], "AssistDividendTracker: Account not excluded");
    ...
    function updateClaimWait(uint256 newClaimWait) external override onlyOwnerOrDeployer {
        require(
            ...
            "AssistDividendTracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(newClaimWait != claimWait, "AssistDividendTracker: Cannot update claimWait to same value");
